    ...
    <>
      ...
      <div className="bg-black text-white min-h-[90vh] relative overflow-hidden flex flex-col items-center selection:bg-amber-500 selection:text-black">
        ...
        <div className="relative z-10 max-w-5xl w-full px-6 pt-20 pb-32 flex flex-col items-center text-center">
          ...
          {/* Headline */}
          <h1
            className={`font-display text-4xl md:text-7xl font-bold leading-[1.1] tracking-tighter transition-all duration-1000 delay-200 ${
          ...
          {/* Paragraph */}
          <div
            className={`mt-10 space-y-6 max-w-2xl transition-all duration-1000 delay-400 ${
              visible ? "opacity-100" : "opacity-0"
            }`}
          >
            <p className="font-body text-xl text-white/80 leading-relaxed font-light">
              RB Hires Consulting helps job seekers find careers and enables
              companies to hire skilled professionals.
            </p>
          </div>

          {/* Actions (FIXED RESPONSIVE BUTTONS) */}
          <div
            className={`mt-12 flex flex-row flex-wrap justify-center items-center gap-3 sm:gap-5 transition-all duration-1000 delay-500 ${
              visible ? "opacity-100" : "opacity-0"
            }`}
          >
            <button className="flex-1 sm:flex-none px-6 sm:px-10 py-4 sm:py-5 bg-amber-500 text-black font-bold rounded-2xl hover:bg-white transition-all active:scale-95 shadow-lg shadow-amber-500/20">
              Contact Us
            </button>

           <button className="flex-1 sm:flex-none px-6 sm:px-10 py-4 sm:py-5 bg-transparent text-white border border-white/20 font-bold rounded-2xl hover:bg-white/5 transition-all whitespace-nowrap">
  Explore Jobs
</button>
          </div>
        </div>
