        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 md:gap-16 items-center mb-20 md:mb-32">
          ...
          <div className="flex flex-col">
            ...
            <h1 className="text-2xl sm:text-3xl md:text-5xl font-black tracking-tighter uppercase italic leading-[0.9] md:leading-[0.8] mb-6 md:mb-8">
              INTERVIEW<br />
              <span className="text-amber-500 underline decoration-white/10 underline-offset-8">
                SURE
              </span>
            </h1>

            <p className="text-gray-400 text-sm sm:text-base md:text-lg leading-relaxed mb-8 md:mb-10 max-w-lg">
              Designed for candidates who want{" "}
              <span className="text-white font-bold italic">
                interviews, not just applications.
              </span>{" "}
              We focus on advanced resume positioning and recruiter outreach to get you in the room.
            </p>

            {/* FEATURES */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 md:gap-y-8 md:gap-x-12 mb-10 md:mb-12">
              {[Users, PhoneCall, Zap, Globe].map((Icon, i) => (
                <div key={i} className="space-y-2">
                  <div className="flex items-center gap-3 text-white">
                    <Icon className="w-4 h-4 text-amber-500" />
                    <span className="text-[11px] font-black uppercase tracking-widest">
                      {["1-on-1 Prep", "Recruiter Reach", "ATS Mastery", "Reporting"][i]}
                    </span>
                  </div>
                  <p className="text-[11px] text-gray-500 font-bold uppercase leading-relaxed">
                    {
                      [
                        "Technical & HR mock sessions with feedback.",
                        "Direct outreach to active hiring managers.",
                        "Advanced LinkedIn & Resume optimization.",
                        "Full interview tracking and funnel analysis."
                      ][i]
                    }
                  </p>
                </div>
  ...
  Secure Your Slot
        ...
        <div className="max-w-4xl mx-auto pt-12 md:pt-20 border-t border-white/5">
          <div className="text-center mb-10 md:mb-16">
            <p className="text-amber-500 text-[10px] font-black uppercase tracking-[0.5em] mb-4">
              RB Hires Intelligence
            </p>
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-black tracking-tighter uppercase italic">
              InterviewSure <span className="text-amber-500">FAQ</span>
            </h2>
          </div>

          <div className="grid gap-3">
            {faqs.map((faq, index) => (
              <div key={index} className="border rounded-2xl border-white/5">
                ...
                >
                  ...
                  <ChevronDown
                    className={`w-4 h-4 transition ${
                      openFaq === index ? "rotate-180 text-amber-500" : "text-gray-600"
                    }`}
                  />
                </button>

                <div className={`${openFaq === index ? "block" : "hidden"} px-6 pb-6 text-gray-500 text-xs font-bold`}>
                  {faq.a}
                </div>
        ...
        {/* GUARANTEE */}
        <div className="mt-20 md:mt-32 p-8 sm:p-10 md:p-16 bg-gradient-to-br from-[#0a0a0a] to-black border border-amber-500/20 rounded-[2rem] md:rounded-[4rem] text-center">
          ...
          <p className="text-gray-500 max-w-2xl mx-auto text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.2em] leading-loose">
            ...
            <span className="text-white block mt-3 text-sm italic">
              RB Hires Consulting will continue interview marketing at{" "}
